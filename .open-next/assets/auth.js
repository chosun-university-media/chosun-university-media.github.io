(() => {
  ...
  const APP_SCRIPT_VERSION = "20260821-daum-password-v1";
  ...
  function bindEvents() {
    ...
    authModalRoot.addEventListener("submit", handleModalSubmit);
  ...
  function renderAccountMenu() {
    ...
    menu.innerHTML = `
      ...
      <button type="button" data-site-account="profile">마이페이지</button>
  ...
  function openProfileModal(message = "", tone = "") {
    authModalRoot.classList.add("is-open");
    authModalRoot.innerHTML = `
      <div class="auth-admin-modal" role="dialog" aria-modal="true" aria-labelledby="profile-title">
        <section class="member-dialog account-dialog">
          <header class="member-dialog-header">
            <div><p>MY ACCOUNT</p><h2 id="profile-title">마이페이지</h2><span>내 계정 정보와 비밀번호를 관리합니다.</span></div>
            <div class="member-dialog-tools compact"><button type="button" data-member-action="close" aria-label="닫기">×</button></div>
          </header>
          <div class="account-dialog-body">
            <dl class="profile-summary account-summary">
              <div><dt>이름</dt><dd>${escapeHtml(currentProfile.name || "-")}</dd></div>
              <div><dt>소속</dt><dd>${escapeHtml(currentProfile.department || "-")}</dd></div>
              <div><dt>아이디</dt><dd>${escapeHtml(currentProfile.username || "-")}</dd></div>
              <div><dt>권한</dt><dd>${currentProfile.role === "admin" ? "관리자" : "일반 회원"}</dd></div>
            </dl>
            ${message ? `<p class="auth-message ${tone || "error"}" role="status">${escapeHtml(message)}</p>` : ""}
            <form class="auth-form" data-password-form="own">
              <label>현재 비밀번호<input name="currentPassword" type="password" autocomplete="current-password" minlength="8" required /></label>
              <label>새 비밀번호<input name="newPassword" type="password" autocomplete="new-password" minlength="8" required /></label>
              <label>새 비밀번호 확인<input name="passwordConfirm" type="password" autocomplete="new-password" minlength="8" required /></label>
              <button class="auth-primary" type="submit">비밀번호 변경</button>
            </form>
          </div>
        </section>
      </div>`;
      ...
      <tr>
        ...
        <td>
          <form class="member-actions" data-member-id="${escapeHtml(member.id)}">
            ...
            <button type="button" data-member-action="reset-password" data-member-id="${escapeHtml(member.id)}" data-member-name="${escapeHtml(member.name || member.username)}" data-member-username="${escapeHtml(member.username)}">비밀번호 재설정</button>
          </form>
        </td>
      </tr>`;
  }
  ...
  async function handleModalSubmit(event) {
    ...
    if (passwordForm) {
      ...
      try {
        if (passwordForm.dataset.passwordForm === "own") await changeOwnPassword(passwordForm);
        if (passwordForm.dataset.passwordForm === "admin-reset") await resetMemberPassword(passwordForm);
      } finally {
        authBusy = false;
    ...
    await handleMemberSubmit(event);
  }
  ...
  async function changeOwnPassword(form) {
    ...
    if (newPassword.length < 8) return openProfileModal("새 비밀번호는 8자 이상이어야 합니다.");
    if (newPassword !== passwordConfirm) return openProfileModal("새 비밀번호가 서로 일치하지 않습니다.");
    if (currentPassword === newPassword) return openProfileModal("현재 비밀번호와 다른 새 비밀번호를 입력해 주세요.");

    const { error: verifyError } = await client.auth.signInWithPassword({
      email: usernameToEmail(currentProfile.username),
      password: currentPassword,
    });
    if (verifyError) return openProfileModal("현재 비밀번호가 올바르지 않습니다.");

    const { error } = await client.auth.updateUser({ password: newPassword });
    if (error) return openProfileModal("비밀번호를 변경하지 못했습니다. 잠시 후 다시 시도해 주세요.");
    openProfileModal("비밀번호를 변경했습니다. 다음 로그인부터 새 비밀번호를 사용해 주세요.", "success");
  }

  function openAdminPasswordReset(button, message = "", tone = "") {
    const member = {
      id: button?.dataset.memberId || "",
      name: button?.dataset.memberName || "회원",
      username: button?.dataset.memberUsername || "",
    };
    authModalRoot.classList.add("is-open");
    authModalRoot.innerHTML = `
      <div class="auth-admin-modal" role="dialog" aria-modal="true" aria-labelledby="reset-title">
        <section class="member-dialog account-dialog">
          <header class="member-dialog-header">
            <div><p>ADMIN RESET</p><h2 id="reset-title">비밀번호 재설정</h2><span>기존 비밀번호는 표시되지 않으며 새 비밀번호로 교체됩니다.</span></div>
            <div class="member-dialog-tools compact"><button type="button" data-member-action="close" aria-label="닫기">×</button></div>
          </header>
          <div class="account-dialog-body">
            <dl class="profile-summary account-summary">
              <div><dt>회원</dt><dd>${escapeHtml(member.name)}</dd></div>
              <div><dt>아이디</dt><dd>${escapeHtml(member.username)}</dd></div>
            </dl>
            ${message ? `<p class="auth-message ${tone || "error"}" role="status">${escapeHtml(message)}</p>` : ""}
            <form class="auth-form" data-password-form="admin-reset">
              <input name="memberId" type="hidden" value="${escapeHtml(member.id)}" />
              <input name="memberName" type="hidden" value="${escapeHtml(member.name)}" />
              <input name="memberUsername" type="hidden" value="${escapeHtml(member.username)}" />
              <label>새 비밀번호<input name="newPassword" type="password" autocomplete="new-password" minlength="8" required /></label>
              <label>새 비밀번호 확인<input name="passwordConfirm" type="password" autocomplete="new-password" minlength="8" required /></label>
              <button class="auth-primary" type="submit">새 비밀번호 적용</button>
              <button class="member-close-text inline" type="button" data-member-action="members">회원 목록으로</button>
            </form>
          </div>
        </section>
      </div>`;
  ...
  async function resetMemberPassword(form) {
    ...
    const memberName = form.elements.namedItem("memberName")?.value || "회원";
    const memberUsername = form.elements.namedItem("memberUsername")?.value || "";
    const newPassword = form.elements.namedItem("newPassword")?.value || "";
    const passwordConfirm = form.elements.namedItem("passwordConfirm")?.value || "";
    const target = { dataset: { memberId, memberName, memberUsername } };
    if (!memberId) return openAdminPasswordReset(target, "회원 정보를 확인하지 못했습니다.");
    if (newPassword.length < 8) return openAdminPasswordReset(target, "새 비밀번호는 8자 이상이어야 합니다.");
    if (newPassword !== passwordConfirm) return openAdminPasswordReset(target, "새 비밀번호가 서로 일치하지 않습니다.");

    const { error } = await client.functions.invoke("admin-reset-password", {
      body: { userId: memberId, newPassword },
    });
    if (error) return openAdminPasswordReset(target, "비밀번호를 재설정하지 못했습니다. 관리자 기능 배포 상태를 확인해 주세요.");
    openAdminPasswordReset(target, `${memberName} 회원의 비밀번호를 재설정했습니다.`, "success");
  }
  ...
  function handleModalClick(event) {
    ...
    if (action === "reset-password") return openAdminPasswordReset(actionButton);
    if (action === "members") return void openMemberModal();
    if (event.target.matches(".auth-admin-modal") || action === "close") closeMemberModal();
  }
